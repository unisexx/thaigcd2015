...
<script type="text/javascript">
	$(function(){
		$('#sign_edit').click(function(){
			...
			$('#show_signature').slideUp("slow",function(){
				$('#sign_frm').slideDown("slow");
			});
			return false;
		});
		...
		$('#avatar_edit').click(function(){
			...
			$('#profile_blog').slideUp("slow",function(){
				$('#edit_profile_form').slideDown("slow");
			});
			return false;
		});
...
<div id="profile">
	<h1>ข้อมูลส่วนตัว <a id="avatar_edit" href="#"><?php if($this->session->userdata('id') == $user->id):?>[แก้ไขข้อมูลส่วนตัว]<?php endif;?></a></h1>
	<div id="profile_blog">
	...
	<br>
		<form method="post" action="users/profile_save/<?php echo $this->session->userdata('id')?>" enctype="multipart/form-data">
			<table class="form">
