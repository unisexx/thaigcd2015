...
class Blogs extends Public_Controller
{
	...
	{
		$data['blogs'] = new blog();
		if(isset($_POST['search'])) $data['blogs']->where('title like','%'.$_POST['search'].'%');
	...
	{
		$data['blogposts'] = new blogpost();
		if(isset($_POST['search'])) $data['blogposts']->where('title like','%'.$_POST['search'].'%');
	...
	function setting()
	{
		if(is_login())
		{
			...
			if($_POST)
			{
				...
				{
					$data['blog']->delete_file('uploads/users/blog/'.$this->session->userdata('id').'/','header');
