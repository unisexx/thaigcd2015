...
class Webboards extends Public_Controller
{
	...
	function category($id)
	{
		...
		$data['webboard_quizs']->where("category_id = $id and stick = 0")->order_by('id','desc')->get_page();
		$data['webboard_quizs_stick'] = new Webboard_quiz();
		$data['webboard_quizs_stick']->where("category_id = $id and stick = 1")->order_by('id','desc')->get_page();
	...
	function reply($topic_id,$quote_id,$type=FALSE)
	{
		if(!is_login()){
			set_notify('error','กรุณาเข้าสู่ระบบ');
		...
		if(!is_login('Administrator')){
			if($data['webboard_quiz']->group_id != 0){
				if(user()->group_id != $data['webboard_quiz']->group_id){
					set_notify('error','กลุ่มของคุณไม่ได้รับอนุญาต');
					$site_redirect = "webboards/view_topic/".$topic_id;
				redirect($site_redirect);
				}
			}
