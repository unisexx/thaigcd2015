...
<div class="search">
	<form method="post">
		<table class="form">
			<tr><th>คำถาม</th><td><input type="text" name="search" value="<?php echo (isset($_POST['search']))?$_POST['search']:'' ?>" /></td><th>หมวดหมู่</th><td><?php echo form_dropdown('category_id',$faqs->category->get_option(),@$_POST['category_id'],'','ทั้งหมด') ?></td><td><input type="submit" value="ค้นหา" /></td></tr>
		</table>
	</form>
</div>
...
<table class="list">
	...
	<tr <?php echo cycle()?>>
		...
		<td><?php echo lang_decode($faq->category->name,'') ?></td>
		<td>
