...
class Newsletters extends Admin_Controller
{
	...
	function save($id=FALSE){
		...
		if($_POST)
		{
			...
			$users = new User();
			$users->where("newsletter like '%".$newsletter->category_id."%'")->get();
			foreach($users as $user)$email_member[] = $user->email;
			...
			// ###### PHPMailer #### 
				...
				$mail->Port = 465;
				$mail->Username = 'thaigcd.morph.go.th@gmail.com';
				$mail->Password = '1212312121';
				$mail->SMTPAuth = true;
				$mail->CharSet = "utf-8";
				$mail->From = "thaigcd.morph.go.th@gmail.com";		//  account e-mail ของเราที่ใช้ในการส่งอีเมล
