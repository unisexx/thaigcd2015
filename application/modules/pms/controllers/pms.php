...
class Pms extends Public_Controller {
	...
	function save($id){
		if($_POST){
			...
			{
				$_POST['file1'] = $software->upload($_FILES['file1'],'uploads/pms/');
			}
			// if($_FILES['file2']['name'])
			// {
				// $_POST['file2'] = $software->upload($_FILES['file2'],'uploads/pms/');
			// }
			// if($_FILES['file3']['name'])
			// {
				// $_POST['file3'] = $software->upload($_FILES['file3'],'uploads/pms/');
			// }
			$pm->from_array($_POST);
			$pm->save();
